...
describe("Media Test Suite", function() {
  ...
  it("List user Media", function(done) {
    ...
    s2sMS.Media.listUserMedia(identityData.uuid, accessToken)
      .then(responseData => {
  ...
  it("Upload user Media", function(done) {
    ...
    s2sMS.Media.uploadFile(
      ...
      .then(responseData => {
        ...
        assert(
          responseData.hasOwnProperty("file_id"),
          `missing file_id from results ${JSON.stringify(responseData)}`
        );
        assert(
          fileName.indexOf(responseData["file_name"]) > -1,
          `missing file_name ${fileName} from results  ${
