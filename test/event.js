...
describe('Event', function() {
  it('list Events ', function(done) {
    ...
    s2sMS.Identity.getIdentity(creds.CPAAS_KEY, creds.email, creds.password).then((identityData)=>{
      s2sMS.Event.listEvents(creds.CPAAS_KEY, identityData.user_uuid, identityData.token).then((responseData)=>{
  ...
  it('Create, update and  Delete Event', function(done) {
    ...
    s2sMS.Identity.getIdentity(creds.CPAAS_KEY, creds.email, creds.password).then((identityData)=>{
      s2sMS.Event.createEvent(creds.CPAAS_KEY, identityData.user_uuid, identityData.token, 'title', {a:1, b:2}
           ...
           s2sMS.Event.updateEvent(creds.CPAAS_KEY, identityData.user_uuid, identityData.token,
