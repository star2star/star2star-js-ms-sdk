...
describe('Task', function() {
  it('Get Task Templates', function(done) {
    ...
    s2sMS.Identity.getIdentity(creds.CPAAS_KEY, creds.email, creds.password).then((identityData)=>{
      s2sMS.Task.getTaskTemplates(creds.CPAAS_KEY, identityData.user_uuid, identityData.token).then((responseData)=>{
  ...
  it('Create / Delete Task Template', function(done) {
    if (!creds.isValid) return done();
    s2sMS.Identity.getIdentity(creds.CPAAS_KEY, creds.email, creds.password).then((identityData)=>{
      s2sMS.Task.createTaskTemplate(creds.CPAAS_KEY, identityData.user_uuid, identityData.token, 'foo', [] ).then((responseData)=>{
          //console.log(identityData.token)
        //console.log(responseData)
        assert(responseData.metadata !== null )
        done();
        s2sMS.Task.deleteTaskTemplate(creds.CPAAS_KEY, identityData.user_uuid, identityData.token, responseData.uuid ).then((d)=>{
        });
  ...
  it('Create, update and  Delete Task', function(done) {
    ...
    s2sMS.Identity.getIdentity(creds.CPAAS_KEY, creds.email, creds.password).then((identityData)=>{
      s2sMS.Task.createTaskObject(creds.CPAAS_KEY, identityData.user_uuid, identityData.token, 'title', [{'title': 'hello'}]
           ...
           s2sMS.Task.updateTaskObject(creds.CPAAS_KEY, identityData.user_uuid, identityData.token,
  ...
  it('add task to task object', function(done) {
    ...
    s2sMS.Identity.getIdentity(creds.CPAAS_KEY, creds.email, creds.password).then((identityData)=>{
      s2sMS.Task.createTaskObject(creds.CPAAS_KEY, identityData.user_uuid, identityData.token, 'title', [{'title': 'hello'}]
            ...
            s2sMS.Task.addTaskToTaskObject(creds.CPAAS_KEY, identityData.user_uuid, identityData.token,
             responseData.uuid, {'title': 'lena'} ).then((newData)=>{
